...
* Start __iam-service__ as standalone server.
  ```
  java -Xms32m -Xmx128m -jar iam-service-1.0.1-SNAPSHOT.jar --spring.config.location=file:application.yml
  ```
* Build Docker Image for target platform.
  ```
  docker build -t iam-service:1.0.1-SNAPSHOT --file Dockerfile.x86_64 .
  docker build -t iam-service:1.0.1-SNAPSHOT --file Dockerfile.armv7l .
  docker build -t iam-service:1.0.1-SNAPSHOT --file Dockerfile.aarch64 .
  ```
